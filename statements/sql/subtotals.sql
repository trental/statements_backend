select statement_type, line_item_order, line_item, amount
from output
union
...
union
select 'cash_flow_statement', 12, 'cash_flow_from_operations', sum(amount) as amount
from (
...
union
select 'cash_flow_statement', 32, 'ending_cash_balances', sum(amount) as amount
from (
    select case when line_item = 'cash_receipts' then amount
                when line_item = 'cash_disbursements' then amount * -1
...
union
select 'balance_sheet', 22, 'current_assets', sum(amount) as amount
from (
...
union
select 'balance_sheet', 37, 'net_fixed_assets', sum(amount) as amount
from (
...
union
select 'balance_sheet', 72, 'shareholders_equity', sum(amount) as amount
from (
...
union
select 'balance_sheet', 73, 'total_liabilties_and_equity', sum(amount) as amount
from (
    select case when line_item = 'accounts_payable' then amount
                when line_item = 'accrued_expenses' then amount
                when line_item = 'current_portion_of_debt' then amount
